...
6. 사용자 목록은 15개 단위 게시판 형식이며, 선택한 등록자의 상세 정보와 등록 당시 유입 경로·A/B 배정 정보를 표시합니다.
7. 승인된 사용자는 읽기 전용 MCP 서버를 통해 같은 등록자·분석 데이터를 자연어로 조회할 수 있습니다.

## 익명 사이트 분석

사용자 화면은 소개 방문, 등록 CTA 클릭, 폼 입력 시작, 검증 오류, 제출, 등록 성공과 완료 화면 방문을 익명 세션 단위로 기록합니다. 일반 분석 이벤트에는 이메일, 전공, 지역 등 개인정보와 폼 입력값을 저장하지 않습니다. 등록 성공 이벤트만 관리자 상세 조회를 위해 등록자 ID를 연결하며, 등록자를 삭제하면 외래 키의 `ON DELETE SET NULL` 동작으로 연결이 제거됩니다.
...
`plugins/art-instructor-data`는 사이트 분석, 등록자 조회·집계와 스프레드시트 정리를 돕는 Codex 플러그인입니다. 정확한 이메일로 등록자를 조회하면 등록 당시 기록된 유입 경로·UTM·A/B 배정도 함께 반환합니다. MCP 도구는 모두 읽기 전용이며 임의 SQL, 등록자 수정·삭제, 분석 초기화 기능을 노출하지 않습니다.
...
기존 운영 데이터베이스에 등록자별 유입 연결 컬럼을 추가할 때는 배포 전에 아래 마이그레이션을 한 번 실행합니다. 과거 이벤트는 등록자 ID를 저장하지 않았으므로 소급 추정하지 않으며, 관리자 상세에서 `기록 없음`으로 표시됩니다.

```powershell
npm run attribution:migrate
```
