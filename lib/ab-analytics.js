...
export function normalizeExperimentRange(value) {
  const parsed = Number.parseInt(String(value ?? ""), 10);
  return ALLOWED_RANGES.has(parsed) ? parsed : 30;
}

export function normalizeExperimentFilter(value) {
  if (typeof value !== "string") return "";
  return value.trim().slice(0, 80);
}

function getExperimentPeriodStart(days, now = Date.now()) {
  const rangeDays = normalizeExperimentRange(days);
  ...
  return new Date(
    Date.UTC(
      ...
      nowInKst.getUTCDate() - (rangeDays - 1),
    ) - kstOffsetMs,
  ).toISOString();
}
...
export function buildExperimentComparison(rows) {
  ...
  const relativeLift = variants.A.conversionRate > 0
    ? Math.round(
      ((variants.B.conversionRate - variants.A.conversionRate)
        / variants.A.conversionRate) * 1000,
    ) / 10
    : null;
...
export async function getAbTestReport(sql, options = {}) {
  const rangeDays = normalizeExperimentRange(options.days);
  const source = normalizeExperimentFilter(options.source);
  const campaign = normalizeExperimentFilter(options.campaign);
  const periodStart = getExperimentPeriodStart(rangeDays);
