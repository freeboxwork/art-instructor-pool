import { getAbTestReport as queryAbTestReport } from "./ab-analytics.js";
import { serializeRegistrationAttribution } from "./registration-attribution.js";
...
export async function getAbTestReport(sql, options = {}) {
  return queryAbTestReport(sql, options);
}
...
export async function getRegistrationByEmail(sql, email) {
  ...
  const rows = await sql`
    ...
    WHERE registration.email_normalized = ${normalizedEmail}
    LIMIT 1
  `;
